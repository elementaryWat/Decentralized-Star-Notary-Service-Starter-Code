...
it('can Create a Star', async() => {
    ...
    const values = {0: name, 1: symbol, name, symbol};
    assert.equal(JSON.stringify(result), JSON.stringify(values))
});
...
// Gasprice 0 makes test to timeout
...
it('can add the star name and star symbol properly', async() => {
    ...
    let name = 'new star';
    let symbol = 'NFT';
    await instance.createStar(name,symbol, starId, {from: user1});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('awesome star 2','AS2', starId2, {from: user2});
